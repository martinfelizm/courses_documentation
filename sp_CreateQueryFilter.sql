USE [Tasaciones]
GO
/****** Object:  StoredProcedure [dbo].[sp_CreateQueryFilter_tasacion]    Script Date: 7/20/2022 10:57:26 AM ******/
SET ANSI_NULLS ON
GO
...
GO
 ALTER 
     ...
     PROCEDURE [dbo].[sp_CreateQueryFilter_tasacion] (@objectIds varchar(200),@tableNewName varchar(max),@initSubstring int = 0, @folderDestino varchar(max))--[dbo].[sp_CreateQueryFilter_tasacion]686625489, 'AccesoriosTasacion',0,'\\OPPCTEMP2\New folder (2)\Controllers\' 
AS
--create procedure ooo as
DECLARE @TableName VARCHAR(8000)=''
DECLARE @TableNameFilter VARCHAR(8000)=''
DECLARE @NameSpace VARCHAR(8000) = 'Tasaciones.Core.QueryFilters' -- Replace 'namespace' with your class namespace
...
WHILE @@FETCH_STATUS = 0
  ...
  --  SET @result = @result + 'using System.ComponentModel.DataAnnotations;' + CHAR(13) + CHAR(13) 
	SET @result = @result + 
   '#pragma warning disable 1591' + CHAR(13)
    ...
    SET @result = @result 		
+ '	
    public class ' + @TableName + 'QueryFilter : BasePostQueryFilter
    {' + CHAR(13)
...
+ '}
    ...
    --PRINT @result
		...
		SET @result = ''
		SET @Archivo = @folderDestino + @TableName + 'QueryFilter.cs'
