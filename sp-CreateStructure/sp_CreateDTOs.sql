USE [Tasaciones]
GO
/****** Object:  StoredProcedure [dbo].[sp_CreateDTOs_tasacion]    Script Date: 7/20/2022 10:56:04 AM ******/
SET ANSI_NULLS ON
GO
...
GO
 ALTER 
   ...
   PROCEDURE [dbo].[sp_CreateDTOs_tasacion] (@objectIds varchar(200),@tableNewName varchar(max),@initSubstring int = 0, @folderDestino varchar(max)) -- [dbo].[sp_CreateDTOs_tasacion]686625489, 'AccesoriosTasacion',0,'\\OPPCTEMP2\New folder (2)\Controllers\' 
AS
--create procedure ooo as
DECLARE @TableName VARCHAR(8000)=''
DECLARE @TableNameFilter VARCHAR(8000)=''
DECLARE @NameSpace VARCHAR(8000) = 'Tasaciones.Core.DTOS' -- Replace 'namespace' with your class namespace
...
WHILE @@FETCH_STATUS = 0
  ...
  --  SET @result = @result + 'using System.ComponentModel.DataAnnotations;' + CHAR(13) + CHAR(13) 
	SET @result = @result + '#pragma warning disable 1591
	using System;
	using System.Collections.Generic;
	using System.Linq;
	using System.Threading.Tasks;' + CHAR(13)+ CHAR(13)
    ...
    SET @result = @result + 'public class ' + @TableName+ 'Dto' + CHAR(13) + '{' + CHAR(13) 
	...
	*/
    ...
    --PRINT @result
		...
		SET @result = ''
		SET @Archivo = @folderDestino + @TableName + 'Dto.cs'
		...
		SET @Sql = 'EXEC master..xp_cmdshell ' + char(39) + 'bcp "'+ @Query +
				   '" queryout "' + @Archivo 
				   + '" -P qYKsDjkaX3 -U Apps_User_Dev -S DINTERNOSRV\DEVCONFISA -c -t  ' + char(39)
